# 无人机艇协同搜索MADDPG强化学习框架

## 1. 框架定位

这是一个为"无人机艇协同搜索陌生区域水面目标控制算法研究"竞赛场景设计的、基于**MADDPG算法**的**多智能体强化学习框架**。

本框架采用MADDPG（Multi-Agent Deep Deterministic Policy Gradient）算法，专门为多智能体协同任务设计，具有以下核心优势：
- **集中训练分散执行**：训练时利用全局信息，执行时各智能体独立决策
- **连续动作空间**：支持精确的速度和航向控制，更适合无人机艇的连续控制
- **协同策略学习**：智能体能够学习复杂的协同策略，避免冲突，提高搜索效率
- **稳定训练过程**：相比传统强化学习算法，MADDPG在多智能体环境中训练更稳定

本框架融合了**传统算法的领域知识**与**MADDPG的协同学习能力**，实现了以下核心优化：
- **边界目标增强检测**：针对比赛中关键的边界目标进行专项检测优化
- **智能体深度协同**：基于MADDPG的集中训练实现真正的多智能体协作
- **渐进式课程学习**：从简单场景逐步提升到实际比赛难度
- **连续动作优化**：精确的速度和转向控制，提升搜索效率
- **目标优先级感知**：智能体可识别高价值目标（高速、边界目标）

开发者可以轻松地在此框架上：
-   **测试新算法**: 替换`maddpg.py`为自己的算法实现
-   **对比性能**: 与MADDPG基线进行性能比较
-   **实验新策略**: 调整奖励函数、观测空间或环境配置
-   **验证优化效果**: 使用 `test_maddpg.py` 独立验证MADDPG功能
...
python main.py
```
默认配置已设为连续动作空间的MADDPG训练模式。若需可视化，请修改`config.py`中的`ENABLE_RENDERING`。

### 2.3 (推荐) 运行MADDPG算法验证测试
```bash
python test_maddpg.py
```
该脚本可以独立测试和验证MADDPG的基本功能、协同性能和相比随机策略的优势。

## 3. 项目文件结构解析

| 文件名 | 主要职责 |
| :--- | :--- |
| `main.py` | **主程序入口**。负责组织MADDPG训练循环，调用环境和算法，并实现**课程学习**策略。 |
| `maddpg.py` | **MADDPG算法核心实现**。包含Actor-Critic网络、经验回放缓冲区、多智能体协同训练逻辑。 |
...
| `config.py` | **中央配置文件**。所有超参数、**MADDPG训练参数**和物理参数都在此定义。 |
| `evaluator.py` | **性能评估器**。用于计算和输出标准化的性能指标（如成功率、覆盖率）。 |
| `test_maddpg.py` | **MADDPG验证脚本**。用于独立测试和验证MADDPG算法的基本功能、协同性能等。 |
| `test_optimizations.py` | **优化验证脚本**。用于验证边界检测、协同、奖励系统等各项优化功能。 |
| `ippo.py` | **原IPPO算法实现**（保留用于对比）。 |
| `README.md` | **开发文档** (本文)。 |
